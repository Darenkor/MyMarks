...
description: "Automate Telegram tasks via MCP tools (Composio): send messages, photos, documents, manage chats, edit/delete messages, forward messages, get updates, and manage bot commands. Always search tools first for current schemas."
risk: unknown
...
Automate Telegram Bot API tasks using MCP tools. This skill covers sending messages, photos, documents, managing chats, editing/deleting messages, forwarding messages, and managing bot commands.

## Workflows

### 1. Send Messages
**When to use**: User wants to send a text message to a Telegram chat

**Tool sequence**:
1. `TELEGRAM_GET_ME` - Verify bot connection [Required first time]
2. `TELEGRAM_SEND_MESSAGE` - Send a text message [Required]

**Key parameters**:
- `chat_id`: Target chat ID or username (e.g., `@channel_name` or numeric ID)
- `text`: Message content
- `parse_mode`: `HTML` or `MarkdownV2` for formatting

**Pitfalls**:
- MarkdownV2 requires escaping special chars: `_ * [ ] ( ) ~ > # + - = | { } . !`
- HTML supports: `<b>`, `<i>`, `<code>`, `<pre>`, `<a href="">`, `<s>`, `<u>`
